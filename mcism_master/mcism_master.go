...
//
// MCISM Master to control server and fetch monitoring info.
//
...
for _, k := range serviceList {
	...
	if checkDuplication != slice00[0] {
		...
		svcTmp.NAME = slice00[1]
			...
			// cb-store
			fmt.Println("===========================Get")
			keyValue, _ := store.Get( string("/mcis/"+ slice00[0]) )
			fmt.Println("<" + keyValue.Key + "> \n" + keyValue.Value)
			fmt.Println("===========================")
			// cb-store
			fmt.Println("===========================List")
			keyValueList, _ := store.GetList("/mcis/"+ string(slice00[0]) +"/", true)
			fmt.Println("=========================== GetList(\"/\", Ascending)")
			for _, ev := range keyValueList {
							fmt.Println("<" + ev.Key + "> \n" + ev.Value)
			}
...
func addServerToService(svcId string, svcName string, provider string, instanceIds []*string, serverIPs []*string) {
	...
	fmt.Println("=========================== Put(...)")
	Key := "/mcis/"+ svcId
	mapA := map[string]string{"name":svcName, "description":"the description", "status":"running", "vm_num":"TBD", "placement_algo":"TBD"}
	...
	for i, v := range serverIPs {
		...
		Key = "/mcis/"+ svcId + "/vm/" + *instanceIds[i]
		//svcName + "/server/"+ provider + "/" + *instanceId + "/" + addserver
		mapA = map[string]string{"name":"vmName", "description":"the description", "status":"running", "public_ip":*v, "domain_name":"TBD", "cloud_id":provider, "location":"TBD", "placement_algo":"TBD", "vmimage_name":"TBD", "vcpu_size":"TBD", "memory_size":"TBD", "disk_size":"TBD", "disk_type":"TBD", "vmimage":"TBD", "vmspec":"TBD", "network":"TBD", "subnet":"TBD", "net_security":"TBD"}
		Val, _ = json.Marshal(mapA)
		err = store.Put(string(Key), string(Val))
		if err != nil {
			cblog.Error(err)
