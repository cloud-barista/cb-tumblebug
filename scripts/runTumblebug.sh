...
if [ "$user_choice" != "1" ]; then
    CONTAINER_ENV="--add-host host.docker.internal:host-gateway -e TB_SPIDER_REST_URL=http://$EP_SPIDER:1024/spider -e TB_DRAGONFLY_REST_URL=http://$EP_DRAGONFLY:9090/dragonfly -e TB_TERRARIUM_REST_URL=http://$EP_TERRARIUM:8888/terrarium -e TB_SELF_ENDPOINT=$EP_TUMBLEBUG:1323"
else
