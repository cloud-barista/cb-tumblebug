...
Key fields:
- **CSP name** (`openstack-new01`): Any unique identifier for this CSP instance (e.g., `my-private-cloud`, `datacenter-seoul`). The name is freely chosen and does not need to contain `openstack`.
- **`cloudPlatform: openstack`**: Explicitly declares the underlying platform. This field drives platform resolution — when set, the CSP name has no effect on platform logic. When omitted (as with standard CSPs like `aws`), the CSP name itself is used as the platform (identity mapping).
