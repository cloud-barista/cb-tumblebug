...
**Key Observations:**
- ✅ **Same IDs allowed**: All three namespaces use Infra Id `web-cluster`, VM Id `frontend-1`, VNet Id `main-vpc`
...
> **Note:** The `Id` field is derived from the user-provided `Name` during resource creation. When you create a namespace with `Name: "production"`, the system sets `Id: "production"` as well. This pattern applies consistently across all resource types in CB-Tumblebug (VNet, Infra, VM, SecurityGroup, etc.).
...
graph TB
    ...
    NodeGroup --> VM[VM<br/>Virtual Machine]
...
Infra (Multi-Cloud Infrastructure) is a container that logically groups VMs from multiple clouds.
...
type InfraInfo struct {
    ...
    Name         string         // Same as Id
    Vm           []VmInfo       // All VMs across nodegroups
    // ...
}
...
5. **KeyValue Store Patterns**
   ...
   - `/ns/{nsId}/infra/{infraId}/vm/{vmId}` - VM
