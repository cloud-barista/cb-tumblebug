...
- Restores a resource stuck in `Failed` back to `Available` when the CSP resource is confirmed alive — and keeps the original failure reason in the Condition message.
...
`SpMetaMissing` and `TbMetaOnly` follow the same non-destructive-diagnosis pattern as `CspResourceMissing` above (see the state matrix); they're omitted from the diagram for readability.
...
| Aspect          | Standard                                                                                                                                                                                                                                                                                                                                                 |
| :-------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Diagnosis       | `GetCspResourceStatus` → `GetResourceSyncState` → `ApplySyncState` (`src/core/resource/common.go`), reused as-is                                                                                                                                                                                                                                         |
| Reconciler      | `reconcile/<type>Reconcile.go`, registered via `RegisterReconciler(model.Str<Type>, ...)`; no option params                                                                                                                                                                                                                                              |
| Restore         | If CSP alive and `ShouldRestoreToAvailable`, restore `ConditionReady` to `Available` — always on. `ConditionSynced` is still recorded from the real sync state (`InSync` or `SpMetaMissing`), never forced to `InSync` just because Ready is being restored                                                                                              |
