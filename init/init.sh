...
if [[ "$1" == "-h" ]] || [[ "$1" == "--help" ]]; then
    ...
    echo "  --openbao, --openbao-only     Register CSP credentials to OpenBao only"
    ...
    echo "  $0 --openbao-only             # Register CSP credentials to OpenBao only"
    ...
    echo "  $0 --key-file /path/to/key    # Use key file for decryption"
    echo ""
