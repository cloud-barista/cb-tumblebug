...
func CreateSubnet(ctx context.Context, nsId string, vNetId string, subnetReq *model.SubnetReq) (model.SubnetInfo, error) {
	...
	if err != nil {
		log.Error().Err(err).Msg("")
		return emptyRet, apierr.Wrap(err, fmt.Sprintf("failed to check if the subnet (%s) exists or not", subnetInfo.Id))
	}
	...
	subnetInfo.Status = model.DeriveStatus(resourceType, subnetInfo.Conditions)
	// Save the status
	...
	if err = clientManager.HandleHttpResponse(restyResp, err); err != nil {
		log.Error().Err(err).Msg("")
		return emptyRet, apierr.Wrap(err, fmt.Sprintf("failed to create subnet '%s'", subnetInfo.Id))
	}
	...
	vNetInfo.Status = model.DeriveStatus(parentResourceType, vNetInfo.Conditions)
...
func markSubnetDeleteFailed(nsId, vNetId, subnetId, subnetKey string, subnetInfo *model.SubnetInfo, cause error) {
	...
	subnetInfo.Status = model.DeriveStatus(model.StrSubnet, subnetInfo.Conditions)
...
func syncSubnetState(nsId string, subnetInfo *model.SubnetInfo, vNetInfo *model.VNetInfo, optPreloadedSubnetStatus *model.CspResourceStatusResponse) (model.SimpleMsg, error) {
	...
	default: // SyncStateInSync, not authorized to restore
		model.SetCondition(&subnetInfo.Conditions, model.ConditionSynced, model.ConditionTrue, model.ReasonAvailable, "Resource is in sync across all layers")
		if model.GetCondition(subnetInfo.Conditions, model.ConditionReady) == nil {
			model.SetCondition(&subnetInfo.Conditions, model.ConditionReady, model.ConditionTrue, model.ReasonAvailable, "")
		}
		ret.Message = fmt.Sprintf("subnet (%s) exists on CSP; metadata is consistent (no action needed)", subnetInfo.Id)
	}
	...
	if subnetKey != "" {
		if sVal, mErr := json.Marshal(subnetInfo); mErr == nil {
			if pErr := kvstore.Put(subnetKey, string(sVal)); pErr != nil {
				log.Warn().Err(pErr).Msg("failed to persist updated subnet info")
			}
		}
	}

	// Update SubnetInfoList in parent vNetInfo and recompute ChildrenReady
	hasSubnetFailures := false
	for i, s := range vNetInfo.SubnetInfoList {
		if s.Id == subnetInfo.Id {
			vNetInfo.SubnetInfoList[i] = *subnetInfo
		}
		if vNetInfo.SubnetInfoList[i].Status == model.ResourceStatusFailed {
			hasSubnetFailures = true
		}
	}

	if hasSubnetFailures {
		model.SetCondition(&vNetInfo.Conditions, model.ConditionChildrenReady, model.ConditionFalse, model.ReasonSubnetFailed, "One or more child subnets missing or failed")
...
func RegisterSubnet(ctx context.Context, nsId string, vNetId string, subnetReq *model.RegisterSubnetReq) (model.SubnetInfo, error) {
	...
	if err = clientManager.HandleHttpResponse(restyResp, err); err != nil {
		log.Error().Err(err).Msg("")
		return emptyRet, apierr.Wrap(err, fmt.Sprintf("failed to register subnet '%s'", subnetInfo.Id))
	}
...
func DeregisterSubnet(nsId string, vNetId string, subnetId string) (model.SimpleMsg, error) {
	...
	if err != nil {
		if apierr.IsNotFound(err) {
			...
			return emptyRet, apierr.Wrap(err, fmt.Sprintf("failed to deregister subnet '%s'", subnetInfo.Id))
		}
	} else {
		ok, err := strconv.ParseBool(spResp.Result)
		if err != nil {
...
func FindSubnetByZone(nsId string, vNetId string, zone string) (subnetId string, subnetZone string, err error) {
	...
	vNetInfo, err := GetVNet(nsId, vNetId)
	if err != nil {
		return "", "", apierr.Wrap(err, fmt.Sprintf("failed to get VNet '%s'", vNetId))
	}
