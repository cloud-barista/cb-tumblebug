...
const (
	...
	DiskError     DiskStatus = "Error" // CSP-reported error state
	// DiskFailed is TB-owned lifecycle failure (e.g. DeletionFailed), aligned with ResourceStatusFailed
	DiskFailed DiskStatus = DiskStatus(ResourceStatusFailed)
)
...
type DataDiskInfo struct {
	...
	// Conditions hold structured state observations (Ready/Synced), K8s-style
