...
function test_sequence_add_k8snodegroup() {
	local CSP=$1
	local REGION=$2
	local POSTFIX=$3
	local TestSetFile=$4
	...
	../13.k8scluster/add-k8snodegroup.sh -c $CSP -r $REGION -n $POSTFIX -f $TestSetFile -x $NUMVM -z $K8SCLUSTERID_ADD
	dozing 1
...
function test_sequence_add_k8snodegroup_allcsp() {
	local CSP=$1
	local REGION=$2
	local POSTFIX=$3
	local TestSetFile=$4
...
if [ "${INDEX}" == "0" ]; then
	...
	for ((cspi = 1; cspi <= INDEXX; cspi++)); do
		...
		for ((cspj = 1; cspj <= INDEXY; cspj++)); do
			...
			echo "[add K8SNODEGROUP] K8SCLUSTERs($((INDEXX * INDEXY))) = Cloud($INDEXX) * Region($INDEXY)"
			echo "- add K8SNODEGROUP in ${CONN_CONFIG[$cspi,$REGION]}"

			echo ""
			echo "[add K8SNODEGROUP object]"
			test_sequence_add_k8snodegroup_allcsp $CSP $REGION $POSTFIX $TestSetFile $NUMVM $K8SCLUSTERID_ADD ${0##*/} &
			dozing 3 
		 done
	done
	wait
	...
	test_sequence_add_k8snodegroup $CSP $REGION $POSTFIX $TestSetFile $NUMVM $K8SCLUSTERID_ADD ${0##*/}
