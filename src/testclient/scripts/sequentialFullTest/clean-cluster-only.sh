...
function clean_sequence_cluster() {
	...
	../13.cluster/force-delete-cluster.sh -c $CSP -r $REGION -n $POSTFIX -f $TestSetFile -x terminate
}
...
function clean_sequence_cluster_allcsp() {
	...
	../13.cluster/force-delete-cluster.sh -c $CSP -r $REGION -n $POSTFIX -f $TestSetFile -x $NUMVM
	#dozing 1
...
if [ "${INDEX}" == "0" ]; then
	...
	for ((cspi = 1; cspi <= INDEXX; cspi++)); do
		...
		for ((cspj = 1; cspj <= INDEXY; cspj++)); do
			...
			clean_sequence_cluster_allcsp $CSP $REGION $POSTFIX $TestSetFile $NUMVM ${0##*/} &
	...
	clean_cluster_sequence $CSP $REGION $POSTFIX $TestSetFile
fi
