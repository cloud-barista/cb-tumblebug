...
function test_sequence_set_k8snodegroup_autoscaling() {
	local CSP=$1
	local REGION=$2
	local POSTFIX=$3
	local TestSetFile=$4
	...
	../13.k8scluster/set-k8snodegroup-autoscaling.sh -c $CSP -r $REGION -n $POSTFIX -f $TestSetFile -x $NUMVM 
	dozing 1
...
function test_sequence_set_k8snodegroup_autoscaling_allcsp() {
	local CSP=$1
	local REGION=$2
	local POSTFIX=$3
	local TestSetFile=$4
...
if [ "${INDEX}" == "0" ]; then
	...
	for ((cspi = 1; cspi <= INDEXX; cspi++)); do
		...
		for ((cspj = 1; cspj <= INDEXY; cspj++)); do
			...
			echo ""
			echo "[set K8SNODEGROUP autoscaling]"
			test_sequence_set_k8snodegroup_autoscaling_allcsp $CSP $REGION $POSTFIX $TestSetFile $NUMVM ${0##*/} &
			dozing 3 
		 done
	done
	wait
	...
	test_sequence_set_k8snodegroup_autoscaling $CSP $REGION $POSTFIX $TestSetFile $NUMVM ${0##*/}
