...
function test_sequence_add_nodegroup() {
	...
	../13.cluster/add-nodegroup.sh -c $CSP -r $REGION -n $POSTFIX -f $TestSetFile -x $NUMVM 
	dozing 1
...
if [ "${INDEX}" == "0" ]; then
	...
	for ((cspi = 1; cspi <= INDEXX; cspi++)); do
		...
		for ((cspj = 1; cspj <= INDEXY; cspj++)); do
			...
			test_sequence_add_nodegroup_allcsp $CSP $REGION $POSTFIX $TestSetFile $NUMVM ${0##*/} &
		 done
	done
	wait
	...
	test_sequence_add_nodegroup $CSP $REGION $POSTFIX $TestSetFile $NUMVM ${0##*/}
