...
function test_sequence_change_nodegroup_autoscale_size() {
	local CSP=$1
	local REGION=$2
	local POSTFIX=$3
	local TestSetFile=$4
	...
	../13.cluster/change-autoscalesize.sh -c $CSP -r $REGION -n $POSTFIX -f $TestSetFile -x $NUMVM 
	dozing 1
...
function test_sequence_change_nodegroup_autoscale_size_allcsp() {
	local CSP=$1
	local REGION=$2
	local POSTFIX=$3
	local TestSetFile=$4
...
if [ "${INDEX}" == "0" ]; then
	...
	for ((cspi = 1; cspi <= INDEXX; cspi++)); do
		...
		for ((cspj = 1; cspj <= INDEXY; cspj++)); do
			...
			echo ""
			echo "[set NODEGROUP autoscale size]"
			test_sequence_change_nodegroup_autoscale_size_allcsp $CSP $REGION $POSTFIX $TestSetFile $NUMVM ${0##*/} &
		 done
	done
	wait
	...
	test_sequence_change_nodegroup_autoscale_size $CSP $REGION $POSTFIX $TestSetFile $NUMVM ${0##*/}
